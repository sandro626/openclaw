...
run_profile() {
  ...
  PROFILE_STATE_HOME="${E2E_STATE_HOME:-$HOME}"
  SESSION_JSONL="${PROFILE_STATE_HOME}/.openclaw-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "${PROFILE_STATE_HOME}/.openclaw-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
