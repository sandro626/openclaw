 ...
 * 注册智能机器人 Webhook 目标
 */
export function registerWeComRobotWebhookTarget(path: string, target: WeComRobotTarget): () => void {
...
async function processRobotMessage(
  ...
  const ctxPayload = core.channel.reply.finalizeInboundContext({
    ...
    RawBody: text,
    CommandBody: text,
    From: isGroup ? `wecom-robot:${account.accountId}:group:${chatId}` : `wecom-robot:${account.accountId}:${senderId}`,
...
async function deliverRobotReply(params: {
  ...
  if (config) {
    const { resolveMentionsFromResponse, buildRobotResponseWithMentions } = await import("./robot-mention.js");
    ...
    if (mentions.length > 0) {
      ...
      console.error(`[WeCom Robot] Resolved ${mentions.length} mention(s): ${mentions.map(m => m.name).join(", ")}`);
    }
  }
