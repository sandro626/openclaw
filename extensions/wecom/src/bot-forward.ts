 ...
 * Supports both 'robots' and 'accounts' configurations.
 */
export function buildRobotNameToInfoMap(cfg: ClawdbotConfig): Map<string, { accountId: string; robotKey: string; name: string }> {
...
export function parseMentionsFromText(
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      const robotInfo = nameToInfo.get(mentionName) ||
                        nameToInfo.get(mentionNameLower);
      ...
      if (dynamicAgentId && dynamicAgentId !== currentAgentId && !seen.has(dynamicAgentId)) {
        ...
        if (targetAccountId && targetAccountId !== currentAccountId) {
          ...
          const info = nameToInfo.get(targetAccountId) || nameToInfo.get(targetAccountId.toLowerCase());
...
export async function forwardToBotSubagent(params: {
  ...
  if (!allowAny && !allowSet.has(normalizedTarget)) {
    log(
      `wecom: bot forward skipped, ${currentAgentId} not allowed to spawn ${targetAgentId}`
    );
  ...
  log(
    `wecom: spawning subagent ${targetAgentId} for bot-to-bot communication via gateway`
  );
  ...
  try {
    ...
    if (result?.runId) {
      log(
        `wecom: subagent ${targetAgentId} spawned successfully, runId=${result.runId}`
      );
      return { success: true };
    } else {
