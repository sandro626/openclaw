...
import type { WeComConfig, WeComRobotConfig, ResolvedWeComAccount, WeComMessageType } from "./types.js";
...
function resolveWeComAccount(params: {
  ...
  const useAccountPath = Boolean((cfg.channels?.wecom as WeComConfig | undefined)?.accounts?.[effectiveAccountId]);
...
export const wecomPlugin: ChannelPlugin<ResolvedWeComAccount> & Record<string, any> = {
  ...
  config: {
    ...
    deleteAccount: ({ cfg, accountId }) => {
      ...
      if (accountId === DEFAULT_ACCOUNT_ID) {
        const { corpId, agentId, agentSecret, token, encodingAESKey, webhookUrl, webhookPath, ...rest } = wecomConfig;
        return {
          ...cfg,
          channels: {
            ...cfg.channels,
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean((cfg.channels?.wecom as WeComConfig | undefined)?.accounts?.[resolvedAccountId]);
  ...
  outbound: {
    ...
    textChunkLimit: 2048,
    sendPayload: async ({ to, payload, accountId, cfg, robotResponseUrl, robotSenderId, robotChatId }) => {
      ...
      if (robotResponseUrl) {
        ...
        if (text.trim()) {
          ...
          const chunks = runtime.channel.text.chunkMarkdownTextWithMode(convertedText, 2048, chunkMode);
          ...
          for (const chunk of chunks) {
            ...
            console.error(`[WeCom] Robot response_url delivery response: ${response.status} ${responseText}`);
            ...
            try {
              ...
              if (errcode && errcode !== 0) {
                console.error(`[WeCom] Robot response_url delivery failed with errcode: ${errcode}, errmsg: ${result.errmsg}`);
      ...
      const targetUser = isGroupChat ? robotChatId! : (robotSenderId || to.replace(/^(user:|group:)/, ""));

      try {
        const { getAccessToken, sendMessage, sendAppchatMessage, uploadMedia, sendImageMessage, sendFileMessage, sendVideoMessage, sendVoiceMessage } = await import("./api.js");
        ...
        for (const chunk of chunks) {
          if (isGroupChat) {
            ...
            try {
              ...
              // Fallback 到私聊发送给发送者
              const errorMsg = appchatError instanceof Error ? appchatError.message : String(appchatError);
              console.error(`[WeCom] Appchat API failed (${errorMsg}), falling back to private message`);
    ...
    sendText: async ({ to, text, accountId, cfg, robotResponseUrl, robotSenderId, robotChatId }) => {
      const runtime = getWeComRuntime();
      const account = resolveWeComAccount({ cfg, accountId });
      ...
      if (robotResponseUrl) {
        ...
        for (const chunk of chunks) {
          ...
          const responseText = await response.text();
          console.error(`[WeCom] Robot response_url text delivery response: ${response.status} ${responseText}`);
          ...
          try {
            ...
            const errcode = Number(result.errcode);
            if (errcode && errcode !== 0) {
              console.error(`[WeCom] Robot response_url text delivery failed with errcode: ${errcode}, errmsg: ${result.errmsg}`);
              // errcode 60140 = invalid response code (response_url已被使用或过期)
              // 需要fallback到应用API
              if (errcode === 60140) {
                console.error("[WeCom] response_url已失效(可能已被使用)，将fallback到应用API");
              ...
              throw new Error(`Robot response_url text delivery failed: ${result.errmsg || errcode}`);
            }
          } catch (parseErr) {
            if (!response.ok) {
      ...
      try {
        ...
        return {
          channel: "wecom",
          messageId: lastMessageId,
          chatId: deliveryMethod === "private-message" ? `user:${robotSenderId || targetUser}` : targetUser,
        };
      } catch (fallbackError) {
        // 所有fallback都失败
        console.error(`[WeCom] All fallback methods failed: ${fallbackError}`);
    ...
    sendMedia: async ({ to, text, mediaUrl, accountId, cfg }) => {
      ...
      try {
        let filename: string;
        let ossResult: { url: string; size: number; key: string; contentType: string } | null = null;
        ...
        if (isLocalFile) {
          ...
          const { uploadMedia, sendImageMessage, sendFileMessage, sendVideoMessage, sendVoiceMessage } = await import("./api.js");
     ...
     * 获取微盘文件列表
     */
    wedriveGetFileList: async ({ userId, spaceId, fatherId, sortType, start, limit, accountId, cfg }) => {
      const account = resolveWeComAccount({ cfg, accountId });
      const { getAccessToken } = await import("./api.js");
     ...
     * 上传文件到微盘
     */
    wedriveUploadFile: async ({ userId, spaceId, fatherId, fileName, fileContent, accountId, cfg }) => {
      const account = resolveWeComAccount({ cfg, accountId });
      const { getAccessToken } = await import("./api.js");
  ...
  gateway: {
        ...
        for (const [accountId, accountConfig] of Object.entries(allAccounts)) {
          ...
          if (appPath && appToken && appAESKey) {
            ...
            const unregister = registerWeComWebhookTarget(appPath, {
              ...
              config: cfg,
              runtime: {
                log: ctx.log?.info ? (msg) => ctx.log?.info?.(`[wecom:${accountId}] ${msg}`) : undefined,
                error: ctx.log?.error ? (msg) => ctx.log?.error?.(`[wecom:${accountId}] ${msg}`) : undefined,
                debug: ctx.log?.debug ? (msg) => ctx.log?.debug?.(`[wecom:${accountId}] ${msg}`) : undefined,
              },
              core,
              token: appToken,
              encodingAESKey: appAESKey,
        ...
        for (const [robotId, robotConfig] of Object.entries(robots)) {
          ...
          if (robotPath && robotToken && robotAESKey) {
            ...
            const unregister = registerWeComRobotWebhookTarget(robotPath, {
              ...
              config: cfg,
              runtime: {
                log: ctx.log?.info ? (msg) => ctx.log?.info?.(`[wecom-robot:${robotId}] ${msg}`) : undefined,
                error: ctx.log?.error ? (msg) => ctx.log?.error?.(`[wecom-robot:${robotId}] ${msg}`) : undefined,
                debug: ctx.log?.debug ? (msg) => ctx.log?.debug?.(`[wecom-robot:${robotId}] ${msg}`) : undefined,
              },
              core,
              token: robotToken,
              encodingAESKey: robotAESKey,
            ...
            unregisters.push(unregister);
            ctx.log?.info(`[${account.accountId}] Registered robot webhook: ${robotPath} (robotId: ${robotId})`);
          }
        }
      }
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (accountId === DEFAULT_ACCOUNT_ID) {
        if (nextWecom.corpId || nextWecom.agentId || nextWecom.agentSecret ||
            nextWecom.token || nextWecom.encodingAESKey) {
