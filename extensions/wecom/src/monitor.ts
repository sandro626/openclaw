...
import type { ResolvedWeComAccount, WeComDecryptedMessage, WeComVerifyRequest, WeComRobotMessage } from "./types.js";
 ...
 * 支持格式: @名称、<@名称>、@账号ID
 */
function parseRobotMentions(text: string, config: ClawdbotConfig): Array<{ raw: string; agentId: string; accountId: string }> {
...
function formatRobotMessageContent(message: WeComRobotMessage): string {
  ...
  switch (msgtype) {
    ...
    case "file":
      ...
      const fileSize = (message as any).file?.filesize ? ` (${((message as any).file.filesize / 1024).toFixed(1)}KB)` : "";
...
async function processRobotMessage(
  ...
  console.error(`[WeCom Robot] Raw text.content hex: ${rawContent ? Buffer.from(rawContent).toString('hex') : 'undefined'}`);
  ...
  console.error(`[WeCom Robot] Agent route resolved: agentId=${route.agentId}, accountId=${route.accountId}`);
  ...
  const keywordToAgent: Record<string, string> = {
    "cto": "cto", "CTO": "cto", "cto助手": "cto", "CTO助手": "cto",
    "dev": "dev", "开发": "dev", "开发工程师": "dev", "元宝": "dev",
    "tester": "tester", "测试": "tester", "测试工程师": "tester", "宝气": "tester",
    "ops": "ops", "运维": "ops", "运维工程师": "ops", "智宝": "ops",
  };
  ...
  if (targetAgents.length > 0) {
    const subagentInstructions = targetAgents.map(agentId => {
...
用户消息:
${text}`;
    console.error(`[WeCom Robot] Added subagent instructions for agents: ${targetAgents.join(', ')}`);
  }
  ...
  const ctxPayload = core.channel.reply.finalizeInboundContext({
    ...
    RawBody: text,
    CommandBody: text,
    From: isGroup ? `wecom-bot:${account.accountId}:group:${chatId}` : `wecom-bot:${account.accountId}:${senderId}`,
  ...
  if (mentions.length > 0) {
    console.error(`[WeCom Robot] Original message has ${mentions.length} mentions, spawning subagents...`);
    // 异步触发子代理，不阻塞CEO的处理
    processBotMentions({
      cfg: config,
      runtime,
  ...
  console.error("[WeCom Robot] Context payload:", JSON.stringify({
    SessionKey: accountAwareSessionKey,
    AccountId: route.accountId,
    ChatType: isGroup ? "group" : "direct",
    SenderId: senderId,
  ...
  try {
    dispatchResult = await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
      ...
      dispatcherOptions: {
        deliver: async (payload, info) => {
          console.error(`[WeCom Robot] Deliver called with kind=${info?.kind}, text length=${payload.text?.length || 0}`);
          console.error(`[WeCom Robot] Payload text content: ${payload.text?.substring(0, 500) || '(empty)'}`);
          ...
          console.error(`[WeCom Robot] Parsed mentions from payload: ${mentions.length}, robotKeys: ${mentionedRobotKeys.join(', ')}`);
        ...
        onError: (err, info) => {
          console.error(`[WeCom Robot] onError called with kind=${info?.kind}, error=${String(err)}`);
          runtime.error?.(`WeCom Robot reply failed: ${String(err)}`);
        },
        onSkip: (payload, info) => {
          console.error(`[WeCom Robot] onSkip called with kind=${info?.kind}, reason=${info?.reason}`);
        },
        onReplyStart: () => {
    ...
    console.error("[WeCom Robot] dispatchReplyWithBufferedBlockDispatcher threw error:", dispatchError);
    throw dispatchError;
  }
  ...
  console.error("[WeCom Robot] Agent reply text collected (length=" + agentReplyText.length + "):", agentReplyText.substring(0, 500));
  if (agentReplyText.trim()) {
    console.error("[WeCom Robot] Checking agent reply for bot mentions:", agentReplyText.substring(0, 200));
 ...
 */
async function readXmlBody(req: IncomingMessage, maxBytes = 1024 * 1024): Promise<{
  ok: boolean;
  encrypt?: string;
      ...
      try {
        ...
        if (raw.trim().startsWith("{") || raw.includes('{"msgid"')) {
          try {
            ...
            const jsonStart = raw.indexOf('{"msgid"') !== -1 ? raw.indexOf('{"msgid"') : raw.indexOf("{");
...
export async function handleWeComWebhookRequest(
  ...
  if (body.ok && body.rawJson) {
    ...
    if (robotMessage) {
      ...
      // 异步处理机器人消息
      processRobotMessage(robotMessage, target)
        .catch((err) => {
...
function formatMessageContent(message: WeComDecryptedMessage): string {
  const { MsgType, Content, PicUrl, MediaId, Format, VoiceText, FileName, FileSize, Label, Location_X, Location_Y, Title, Description, Url } = message;
...
async function processMessage(
  ...
  console.error("[WeCom Debug] Processing message from:", senderId, "type:", MsgType, "content:", text);
  ...
  const ctxPayload = core.channel.reply.finalizeInboundContext({
    ...
    RawBody: text,
    CommandBody: text,
    From: isGroup ? `wecom:${account.accountId}:group:${chatId}` : `wecom:${account.accountId}:${senderId}`,
  ...
  const { queuedFinal, counts } = await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
    ...
    dispatcherOptions: {
      deliver: async (payload, info) => {
        console.error(`[WeCom] Deliver called with kind=${info?.kind}, text length=${payload.text?.length || 0}`);
