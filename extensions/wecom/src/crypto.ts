...
const WECOM_AES_KEY = Buffer.from(
  "0123456789abcdefghijkmnopqrstuvwxyz1234567890abc",
  "utf8",
);
...
export class WeComCrypto {
  ...
  decrypt(encryptedMsg: string): { message: string; appId: string } {
    ...
    // AES-256-CBC 解密
    const decipher = crypto.createDecipheriv("aes-256-cbc", this.encodingAESKey, this.encodingAESKey.slice(0, 16));
  ...
  encrypt(message: string): string {
    ...
    // AES-256-CBC 加密
    const cipher = crypto.createCipheriv("aes-256-cbc", this.encodingAESKey, this.encodingAESKey.slice(0, 16));
