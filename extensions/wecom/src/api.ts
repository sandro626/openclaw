...
import type { ResolvedWeComAccount, WeComApiResponse, WeComSendParams, WeComAccessTokenResponse, WeComMediaUploadResponse } from "./types.js";
...
export async function sendMessage(
  ...
  fetcher?: typeof fetch,
): Promise<{ messageId: string; invalidUsers?: string[]; invalidParties?: string[]; invalidTags?: string[] }> {
...
export async function uploadMedia(
  ...
  formData.append("media", new Blob([new Uint8Array(media)]), filename || `media.${type === "image" ? "jpg" : type}`);
