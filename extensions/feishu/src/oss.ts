...
export async function uploadBufferToOSS(
  ...
  try {
    ...
    console.error("[Feishu OSS] Upload failed:", err);
    return null;
  }
}
...
export async function uploadUrlToOSS(
  ...
  try {
    ...
    console.error("[Feishu OSS] Upload from URL failed:", err);
    return null;
  }
}
...
function generateKey(filename: string): string {
  ...
  const prefix = ossConfig?.uploadPath ? ossConfig.uploadPath.replace(/\/$/, "") : "feishu";
