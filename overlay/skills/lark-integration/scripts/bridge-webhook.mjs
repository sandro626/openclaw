 ...
 *
 * Usage:
 *   FEISHU_APP_ID=<your-app-id> WEBHOOK_PORT=3000 node bridge-webhook.mjs
 */
...
const OPENCLAW_HOME = resolve(process.env.OPENCLAW_HOME || "~/.openclaw");
...
const OPENCLAW_CONFIG_PATH = resolve(
  process.env.OPENCLAW_CONFIG_PATH || `${OPENCLAW_HOME}/openclaw.json`,
);
...
try {
  ...
  console.error(`[FATAL] Failed to load OpenClaw config from ${OPENCLAW_CONFIG_PATH}`);
