---
name: memory
title: "Memory - Hippocampus System"
...
- Compatibility names kept for runtime migration: `hippocampus` agent id, `hippocampus-*` cron names, `HIPPOCAMPUS_CORE.md`
- When writing new docs, scripts, or config, prefer `memory` for the skill/package name and keep `hippocampus` only for those legacy-compatible runtime surfaces
...
> `${WORKSPACE:-$HOME/.openclaw/workspace}/brain-dashboard.html`

All scripts use `$WORKSPACE` (default: `${OPENCLAW_WORKSPACE:-$HOME/.openclaw/workspace}`).
...
```bash
# Encoding every 3 hours (legacy-compatible cron name)
openclaw cron add --name hippocampus-encoding \
  --cron "0 0,3,6,9,12,15,18,21 * * *" \
  --session isolated \
  --agent-turn "Run memory encoding with semantic reinforcement..."

# Daily decay at 3 AM (legacy-compatible cron name)
...
This bridges the memory index (`index.json`) with OpenClaw's RAG (`memory_search`).
...
1. Run `${MEMORY_SKILL_DIR:-/path/to/overlay/skills/memory}/scripts/load-core.sh`
...
Track memory activity over time for analytics and debugging:
...
Events append to `${WORKSPACE:-$HOME/.openclaw/workspace}/memory/brain-events.jsonl`:
...
The log `type` stays `hippocampus` for legacy dashboard compatibility.
...
_Memory is identity. Text > Brain. If you don't write it down, you lose it._
