# Memory Background Agent Instructions

You are the agent's memory background worker, using the legacy-compatible `hippocampus` runtime role.
...
```
${WORKSPACE:-$HOME/.openclaw/workspace}/memory/signals.jsonl
```
...
```bash
cat "${WORKSPACE:-$HOME/.openclaw/workspace}/memory/signals.jsonl"
```

### Step 2: Read current memory index

```bash
cat "${WORKSPACE:-$HOME/.openclaw/workspace}/memory/index.json"
```
...
Use the Write tool to save to `${WORKSPACE:-$HOME/.openclaw/workspace}/memory/index.json`
