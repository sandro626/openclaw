...
if [ "$HAS_HIPPOCAMPUS" != "true" ]; then
    echo "❌ No memory data found at $HIPPOCAMPUS_FILE"
    exit 1
fi
    ...
    cat >> "$OUTPUT_FILE" << 'INSTALL_HIPPO'
        <div class="card">
            <div class="install-prompt">
                <div class="icon">🧠</div>
                <p><strong>Memory (Hippocampus)</strong> not installed</p>
                <p>Add memory formation & recall</p>
                <code>clawdhub install hippocampus</code>
                <p class="help-text">Upstream registry still uses the legacy <code>hippocampus</code> alias.</p>
            </div>
        </div>
INSTALL_HIPPO
fi
