          ...
          {steps.map((_, index) => (
            <div
              ...
              className={`w-8 h-8 rounded-full flex items-center justify-center ${
                index + 1 <= step ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-500"
              }`}
            >
              {index + 1}
            </div>
    ...
    <div className="bg-white rounded-lg shadow p-6 max-w-md mx-auto">
      ...
      {showSettings && (
        <div className="mb-4 p-4 bg-gray-50 rounded-lg">
              ...
              {[0.1, 0.5, 1.0].map((value) => (
                <button
                  ...
                  className={`px-3 py-1 rounded ${
                    slippage === value ? "bg-blue-500 text-white" : "bg-white border"
                  }`}
                >
                  {value}%
                </button>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Transaction Deadline</label>
            <input
              type="number"
              value={deadline}
