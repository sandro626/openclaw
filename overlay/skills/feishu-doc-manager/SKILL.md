...
```bash
# Clone into your private overlay source, not the runtime workspace
cd overlay/skills
git clone https://github.com/Shuai-DaiDai/feishu-doc-manager.git
```

部署时通过 `skills.load.extraDirs` 或 bundle 中的 `overlay/skills` 装配，不要把源码直接写进 `~/.openclaw/workspace/*`。
