...
```bash
# Keep the source under overlay/skills and enable it via config
openclaw gateway config.patch --raw '{
  "skills": {
    "load": {
      "extraDirs": ["/path/to/repo/overlay/skills"]
    },
    "entries": {
...
```bash
overlay/skills/agent-council/scripts/create-agent.sh \
  --name "Watson" \
  --id "watson" \
  --emoji "🔬" \
...
```bash
python3 overlay/skills/agent-council/scripts/setup-channel.py \
  --name research \
...
```bash
python3 overlay/skills/agent-council/scripts/rename-channel.py \
  --id 1234567890 \
  --old-name old-name \
  --new-name new-name
```
...
sessions_send({
  label: "watson",
  message:
    "Research competitor X: products, pricing, market position. Write findings to memory/research-X.md",
});
