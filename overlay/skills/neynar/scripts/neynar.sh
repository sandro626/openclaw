...
CONFIG_FILE="${NEYNAR_CONFIG:-$OPENCLAW_HOME/secrets/neynar/config.json}"
...
if [[ -f "$CONFIG_FILE" ]]; then
    ...
    SIGNER_UUID="${SIGNER_UUID:-$(jq -r '.signerUuid // empty' "$CONFIG_FILE")}"
fi

if [[ -z "$API_KEY" ]]; then
    echo "Error: NEYNAR_API_KEY not set" >&2
    echo "Set NEYNAR_API_KEY / NEYNAR_SIGNER_UUID or create $CONFIG_FILE" >&2
    exit 1
fi
