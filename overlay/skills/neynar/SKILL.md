    ...
    "openclaw":
      {
        ...
        "requires": { "bins": ["curl", "jq"], "env": ["NEYNAR_API_KEY"] },
      },
  }
---
...
2. Export credentials or provide a config file:

```bash
export NEYNAR_API_KEY="<your-neynar-api-key>"
export NEYNAR_SIGNER_UUID="<your-signer-uuid>"   # optional, only for posting
# optional override:
export NEYNAR_CONFIG="${OPENCLAW_HOME:-$HOME/.openclaw}/secrets/neynar/config.json"
```

**Note**: `signerUuid` is only required for posting casts. Get one via Neynar's signer management. If you prefer file-based config, use `NEYNAR_CONFIG` and keep it outside git.
