...
- Veil keys: `${VEIL_DIR:-$OPENCLAW_HOME/secrets/veil}/.env.veil` _(chmod 600)_
- Veil RPC config: `${VEIL_DIR:-$OPENCLAW_HOME/secrets/veil}/.env`
- Bankr config: `${BANKR_CONFIG:-~/.bankr/config.json}`
...
- `${VEIL_DIR:-$OPENCLAW_HOME/secrets/veil}/.env` _(preferred)_
...
mkdir -p "$VEIL_DIR"
cat > "$VEIL_DIR/.env" << 'EOF'
RPC_URL=https://base-mainnet.g.alchemy.com/v2/<your-rpc-key>
EOF
...
- For **Bankr signing**, this skill uses Bankr’s Agent API via `BANKR_CONFIG` or Bankr CLI defaults such as `~/.bankr/config.json`.
